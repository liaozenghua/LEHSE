...
MODEL_MAP = {'bert-base': '../bert', 'deberta': '../deberta'}
...
class RerankDataset(Dataset):
    def __init__(self, args, data_file, pos_label=True):
        ...
        with open(self.args.step_goal_file, "r", encoding="utf-8") as f:
            ...
            self.step_goal={}
            for x in self.step_goal:
                if x['task'] not in self.step_goal.keys():
                    self.step_goal[x['task']]=x
                elif x['task']+'1' not in self.step_goal.keys():
                    self.step_goal[x['task']+'1']=x
                elif x['task']+'2' not in self.step_goal.keys():
                    self.step_goal[x['task']+'2']=x
                elif x['task']+'3' not in self.step_goal.keys():
                    self.step_goal[x['task']+'3']=x
                elif x['task']+'4' not in self.step_goal.keys():
                    self.step_goal[x['task']+'4']=x
                elif x['task']+'5' not in self.step_goal.keys():
                    self.step_goal[x['task']+'5']=x
    ...
    def get_surr_context(self, step):
        ...
        for x in step_context:
            ...
            _t.append(x)
        step_context = _t
        if step in step_context:
            step_context=step_context
        elif step in self.step_goal[self.step2goal[step]+'1']['caption']:
            step_context=self.step_goal[self.step2goal[step]+'1']['caption']
        elif step in self.step_goal[self.step2goal[step]+'2']['caption']:
            step_context=self.step_goal[self.step2goal[step]+'2']['caption']
        elif step in self.step_goal[self.step2goal[step]+'3']['caption']:
            step_context=self.step_goal[self.step2goal[step]+'3']['caption']
        elif step in self.step_goal[self.step2goal[step]+'4']['caption']:
            step_context=self.step_goal[self.step2goal[step]+'4']['caption']
        elif step in self.step_goal[self.step2goal[step]+'5']['caption']:
            step_context=self.step_goal[self.step2goal[step]+'5']['caption']
...
def run(args):
    ...
    train_data = DataLoader(dataset=train_ds, batch_size=args.bs, shuffle=True, collate_fn=collate_fn, num_workers=0)
    dev_data = DataLoader(dataset=dev_ds, batch_size=args.val_bs, collate_fn=collate_fn, num_workers=0)
...
def config():
    ...
    parser.add_argument('--train_file', default='../data/gold.rerank.org.t30.train.json')
    parser.add_argument('--dev_file', default='../data/gold.rerank.org.t30.dev.json')
    parser.add_argument('--save_path', default='../model/deberta.reranker.pt')
    parser.add_argument('--gold_step_goal_para_score', help='file that store the retrieval score',
                        default='../data/gold.para.base.all.score')
